@extends('layouts.base')

@section('title', "Licitación #{$licitacion->codigo_licitacion}")

@section('content')
<div class="d-flex justify-content-between align-items-center mb-3">
    <h4 class="mb-0 fw-bold" style="color: var(--color-primary);">
        <i class="bi bi-file-text me-2"></i>Licitación #{{ $licitacion->codigo_licitacion }}
    </h4>
    <div class="d-flex gap-2">
        <a href="{{ route('licitaciones.index') }}" class="btn btn-outline-secondary btn-sm">
            <i class="bi bi-arrow-left me-1"></i>Volver
        </a>
        @if (Auth::user()?->esAdmin())
            <a href="{{ route('licitaciones.edit', $licitacion) }}" class="btn btn-warning btn-sm">
                <i class="bi bi-pencil me-1"></i>Editar
            </a>
        @endif
    </div>
</div>

<div class="card shadow-sm border">
    <div class="card-body p-4">
        <div class="row g-3">
            {{-- Objeto --}}
            <div class="col-12">
                <label class="text-muted small text-uppercase fw-semibold">Objeto</label>
                <p class="fw-semibold mb-0 fs-5" style="color: var(--color-primary);">{{ $licitacion->objeto }}</p>
            </div>

            {{-- Actividad --}}
            @if ($licitacion->actividad)
                <div class="col-12">
                    <label class="text-muted small text-uppercase fw-semibold">Actividad</label>
                    <p class="mb-0">{{ $licitacion->actividad }}</p>
                </div>
            @endif

            {{-- Descripción --}}
            <div class="col-12">
                <label class="text-muted small text-uppercase fw-semibold">Descripción / Alcance</label>
                <p class="mb-0">{{ $licitacion->descripcion }}</p>
            </div>

            <hr class="delimitadorSuperior">

            {{-- Estado --}}
            <div class="col-md-4">
                <label class="text-muted small text-uppercase fw-semibold">Estado</label>
                <p class="mb-0">
                    @php
                        $color = \App\Models\Proceso::COLORES_ESTADO[$licitacion->estado] ?? 'bg-secondary';
                    @endphp
                    <span class="badge {{ $color }}">{{ $licitacion->estado ?? 'Borrador' }}</span>
                </p>
            </div>

            {{-- Presupuesto y moneda --}}
            <div class="col-md-4">
                <label class="text-muted small text-uppercase fw-semibold">Presupuesto</label>
                <p class="h5 mb-0 fw-bold">{{ $licitacion->presupuesto_formateado }}</p>
            </div>
            <div class="col-md-4">
                <label class="text-muted small text-uppercase fw-semibold">Moneda</label>
                <p class="mb-0">{{ $licitacion->moneda }}</p>
            </div>

            <hr class="delimitadorSuperior">

            {{-- Responsable --}}
            <div class="col-md-4">
                <label class="text-muted small text-uppercase fw-semibold">Responsable</label>
                <p class="mb-0">{{ $licitacion->responsable?->nombre ?? '—' }}</p>
            </div>

            {{-- Fechas --}}
            <div class="col-md-4">
                <label class="text-muted small text-uppercase fw-semibold">Fecha de inicio</label>
                <p class="mb-0">{{ $licitacion->fecha_inicio->format('d/m/Y') }}</p>
                <small class="text-muted">{{ $licitacion->hora_inicio }}</small>
            </div>
            <div class="col-md-4">
                <label class="text-muted small text-uppercase fw-semibold">Fecha de cierre</label>
                <p class="mb-0">{{ $licitacion->fecha_cierre->format('d/m/Y') }}</p>
                <small class="text-muted">{{ $licitacion->hora_cierre }}</small>
            </div>

            <hr class="delimitadorSuperior">

            {{-- Metadatos --}}
            <div class="col-md-6">
                <label class="text-muted small text-uppercase fw-semibold">Creado por</label>
                <p class="mb-0 small">{{ $licitacion->creador?->name ?? '—' }}</p>
            </div>
            <div class="col-md-6">
                <label class="text-muted small text-uppercase fw-semibold">Creado</label>
                <p class="mb-0 small">{{ $licitacion->created_at?->format('d/m/Y H:i') ?? '—' }}</p>
            </div>
        </div>
    </div>
</div>
@endsection
